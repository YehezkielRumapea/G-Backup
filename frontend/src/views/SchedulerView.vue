...
const runJobNow = async (jobId) => {
  ...
  console.log(`Mencoba menjalankan job ID: ${jobId}`);
  
  try {
    // Panggil endpoint API untuk menjalankan job
    ...
    alert("Gagal menjalankan job: " + (error.response?.data?.error || "Endpoint belum tersedia atau terjadi error di backend"));
  }
};

// --- COMPUTED & HELPERS (Tidak perlu diubah) ---
...
const getStatusClass = (status) => {
  ...
  if (status === 'FAIL' || status === 'FAILED' || status === 'ERROR') return 'status-failed';
        ...
        <table class="data-table">
          ...
          <tbody>
            ...
            <tr v-for="job in filteredJobs" :key="job.id">
              <td><strong>{{ job.name }}</strong><br><small>{{ job.jobType }} to {{ job.remoteName }}</small></td>
              <td>{{ formatTimestamp(job.lastRun) }}</td>
              <td>
                <span :class="['status-badge', getStatusClass(job.statusQueue)]">
                  {{ job.statusQueue }}
                </span>
              </td>
...
<style scoped>
/* Style ini disalin dari JobStatusView.vue dan akan berfungsi sama */
