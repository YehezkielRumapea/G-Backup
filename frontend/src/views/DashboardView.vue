...
const backupForm = ref({
    ...
    ssh_user: 'idven', 
    remote_name: '',
    destination_path: '/gbackup_files/',
    schedule_cron: '',
    db_name: '',
    db_user: '',
    db_pass: '',
...
const fetchData = async () => {
    ...
    try {
        ...
        // --- BLOK YANG DIPERBAIKI (HANYA SATU KALI) ---
...
const startRestore = async () => {
    ...
    try {
        const response = await apiClient.post('/jobs/restore', payload);
...
const nextJob = computed(() => {
    if (!Array.isArray(jobs.value)) return null; 
...
.quick-actions .action-buttons { 
    display: grid; 
    grid-template-columns: 1fr 1fr; /* <-- INI KUNCINYA */
    gap: 15px; 
}
.restore-btn { 
